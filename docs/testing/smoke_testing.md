...
- [ ] Verify that `OpenProject` user and group are created with user `OpenProject` as admin of the group.
- [ ] Verify that `OpenProjectNoAutomaticProjectFolders` group is also created with user `OpenProject` as admin.
- [ ] Try deleting `OpenProject` user and group, those should not be deleted.
- [ ] Try deleting `OpenProjectNoAutomaticProjectFolders` group, those also should not be deleted.
