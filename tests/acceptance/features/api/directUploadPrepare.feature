Feature: API endpoint to prepare direct upload
  ...
  Scenario Outline: Get a direct-upload token for a folder
    ...
    When user "Alice" sends a POST request to the direct-upload endpoint with the ID of <folder>
  ...
  Scenario: Try to get a direct-upload token for the root folder
    When user "Alice" sends a POST request to the direct-upload endpoint with the ID of "/"
    Then the HTTP status code should be "200"
    And the data of the response should match
    """"
    {
  ...
  Scenario Outline: Get a direct-upload token for a folder received as share
    ...
    When user "Alice" sends a POST request to the direct-upload endpoint with the ID of "/toShare"
    Then the HTTP status code should be "200"
    And the data of the response should match
    """"
    {
  ...
  Scenario: Try to get a direct-upload token for a file
    ...
    When user "Alice" sends a POST request to the direct-upload endpoint with the ID of "/file.txt"
  ...
  Scenario: Try to get a direct-upload token for a non existing folder-id
    When user "Alice" sends a POST request to the direct-upload endpoint with the ID "999999999"
    Then the HTTP status code should be "404"
    And the data of the response should match
    """"
    {
  ...
  Scenario: Try to get token as non-existent user
    When user "test" sends a POST request to the direct-upload endpoint with the ID "123"
