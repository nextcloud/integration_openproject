...
cat >request_body_1_op_create_storage.json <<EOF
{
...
create_storage_response=$(curl -X POST -u${OP_ADMIN_USERNAME}:${OP_ADMIN_PASSWORD} \
  ...
  -H 'Content-Type: application/json' \
  -H 'X-Requested-With: XMLHttpRequest' \
  -d @request_body_1_op_create_storage.json
)
if [[ $INTEGRATION_SETUP_DEBUG != "true"  ]] ; then rm request_body_1_op_create_storage.json; fi
...
cat >request_body_2_nc_create_storage.json <<EOF
{
  "values":{
...
nextcloud_information_response=$(curl -s -X${setup_method} -u${NC_ADMIN_USERNAME}:${NC_ADMIN_PASSWORD} "${NC_INTEGRATION_BASE_URL}/setup" \
  -H 'Content-Type: application/json' \
  -d @request_body_2_nc_create_storage.json
)
if [[ $INTEGRATION_SETUP_DEBUG != "true"  ]] ; then rm request_body_2_nc_create_storage.json; fi
...
cat >request_body_3_op_set_nc_oauth_details.json <<EOF
{
...
set_nextcloud_to_storage_response=$(curl -s -X POST -u${OP_ADMIN_USERNAME}:${OP_ADMIN_PASSWORD} \
                                  ...
                                  -H 'Content-Type: application/json' \
                                  -H 'X-Requested-With: XMLHttpRequest' \
                                  -d @request_body_3_op_set_nc_oauth_details.json
)
if [[ $INTEGRATION_SETUP_DEBUG != "true"  ]] ; then rm request_body_3_op_set_nc_oauth_details.json; fi
...
# save the application password to OpenProject
  ...
  cat >request_body_4_op_set_project_folder_app_password.json <<EOF
  {
...
EOF
  save_app_password_response=$(curl -s -X PATCH -u${OP_ADMIN_USERNAME}:${OP_ADMIN_PASSWORD} \
                                    ...
                                    -H 'Content-Type: application/json' \
                                    -H 'X-Requested-With: XMLHttpRequest' \
                                    -d @request_body_4_op_set_project_folder_app_password.json
    )
    if [[ $INTEGRATION_SETUP_DEBUG != "true"  ]] ; then rm request_body_4_op_set_project_folder_app_password.json; fi
